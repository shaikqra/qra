...
const SRC: Record<FieldSource, { label: string; cls: string }> = {
  profile: { label: "Your profile", cls: "bg-zinc-100 text-zinc-600" },
  po: { label: "From your PO", cls: "bg-sky-50 text-sky-700" },
  draft: { label: "Qra draft — confirm", cls: "bg-amber-50 text-amber-700" },
  default: { label: "Your default", cls: "bg-zinc-100 text-zinc-600" },
};
...
export function ProvenancePanel({ rows }: { rows: ProvenanceRow[] }) {
  ...
  return (
    <section>
      <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-zinc-500">Where your data came from</h2>
        ...
        <div className="flex flex-col divide-y divide-zinc-100">
          {rows.map((r, i) => (
            <div key={i} className="flex items-center gap-3 py-2">
              <div className="w-24 shrink-0 text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                {r.label}
              </div>
              <div className="min-w-0 flex-1 truncate text-sm text-zinc-800">{r.value}</div>
